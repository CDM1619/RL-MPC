# -*- coding: utf-8 -*-
"""
Created on Sat Nov 13 13:19:03 2021
...
from CarCombat_test import CarCombatEnv
    ...
    for episode in range(Episode):
        ...
        for i in range(step):
           ...
           #      action = agent.get_action_test(state)
            ...
            # next_state, reward, done, _, o_x, o_y, reward__ = env.step(action * action_mean + action_bias, action_2 * action_mean + action_bias,episode,i,o_x,o_y)
        ...
        if episode+1 == Episode:
           ...
           #画矩形用来画小车           
            ...
            for i in range(0,len(x_a)-1): 
                ...
                left=min(min(min(x_a),min(x_b)),min(min(y_a),min(y_b)))-40
                right=max(max(max(x_a),max(x_b)),max(max(y_a),max(y_b)))+40
                plt.xlim(left-80,right+80)
                plt.ylim(left,right)               
                # plt.xlim(-400,1000)
                # plt.ylim(-400,150)
                ...
                plt.text(mid_left, plt.ylim()[1]+20, 'Car A : '+str(round(w_a*2,1)), color='royalblue',ha='center',va='center',weight='heavy',fontsize=12,zorder=1000)
                plt.text((plt.xlim()[1]+plt.xlim()[0])/2, plt.ylim()[1]+20, ':', color='black',ha='center',va='center',weight='heavy',fontsize=12,zorder=1000)
                plt.text(mid_right, plt.ylim()[1]+20, 'Car B : '+str(round(w_b*2,1)), color='red',ha='center',va='center',weight='heavy',fontsize=12,zorder=1000)
